...
/*!
 * \class GameServer
 * \brief Сервер для многопользовательской игры "Камень, ножницы, бумага".
 *
 * Класс реализует WebSocket-сервер для управления игровыми комнатами, обработки ходов игроков
 * и синхронизации состояния между клиентами. Основная функциональность:
 * - Создание/удаление комнат
 * - Определение победителя раунда
 * - Рассылка уведомлений клиентам
 * - Обработка сетевых событий (подключение/отключение)
 */
...
public:
    /*!
     * \brief Конструктор GameServer.
     * \param port Порт, на котором запускается сервер.
     * \param parent Родительский объект (по умолчанию nullptr).
     *
     * Инициализирует WebSocket-сервер и запускает его на указанном порту.
     * Если сервер успешно запущен, подключает обработчик новых соединений.
     * В случае ошибки сервер не запускается, и выводится сообщение в лог.
     */
...
private:
    ...
    */
    ...
    /*!
     * \brief Обрабатывает входящие сообщения от клиентов.
     * \param message JSON-сообщение в строковом формате.
     *
     * Выполняет разбор сообщения, проверяет его корректность и обрабатывает команды:
     * - "create_room": создаёт новую комнату и уведомляет клиентов.
     * - "join_room": добавляет игрока в комнату, если в ней есть свободное место.
     * - "play": передаёт ход в метод handleGameMove().
     * - "exit": удаляет комнату и уведомляет клиентов.
     * - "restart": перезапускает игру в комнате.
     *
     * Если сообщение некорректно, сервер игнорирует его.
     */
     ...
     * с помощью функции `determineWinner` и отправляется результат игры обоим участникам комнаты.
     *
