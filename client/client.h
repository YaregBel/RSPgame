...
/*!
 * \brief Класс Client — WebSocket-клиент для игры "Камень, ножницы, бумага".
 *
 * Этот класс управляет соединением с сервером, отправляет запросы и обрабатывает входящие сообщения.
 * Он обеспечивает взаимодействие клиента с сервером, позволяя создавать и присоединяться к игровым комнатам,
 * отправлять игровые действия и получать результаты.
 *
 * Основные возможности:
 * - Устанавливает WebSocket-соединение с сервером.
 * - Создаёт и присоединяется к игровым комнатам.
 * - Отправляет выбор игрока (камень, ножницы, бумага).
 * - Обрабатывает ответы сервера и испускает соответствующие сигналы.
 * - Обрабатывает отключение от сервера и уведомляет об этом.
 *
 * \signals
 * - \a roomJoined(const QString&) — вызывается при успешном создании или входе в комнату.
 * - \a newRoomAvailable(const QString&) — вызывается при появлении новой доступной комнаты.
 * - \a errorReceived(const QString&) — вызывается при получении ошибки от сервера.
 * - \a roomRemoved(const QString&) — вызывается при удалении комнаты.
 * - \a gameResultReceived(const QString&) — вызывается при получении результата игры.
 * - \a opponentLeft() — вызывается, если соперник покинул игру.
 * - \a connectionLost() — вызывается при разрыве соединения с сервером.
 */
